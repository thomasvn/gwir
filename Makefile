...
mac:
	GOOS=darwin GOARCH=arm64 go build -o builds/gwir ./cmd/gwir
	tar -czvf builds/gwir.macos-arm64.tar.gz -C builds gwir
	rm -rf builds/gwir

mac_amd64:
	GOOS=darwin GOARCH=amd64 go build -o builds/gwir ./cmd/gwir
	tar -czvf builds/gwir.macos-amd64.tar.gz -C builds gwir
	rm -rf builds/gwir

linux:
	GOOS=linux GOARCH=amd64 go build -o builds/gwir ./cmd/gwir
	tar -czvf builds/gwir.linux-amd64.tar.gz -C builds gwir
	rm -rf builds/gwir

windows:
	GOOS=windows GOARCH=amd64 go build -o builds/gwir.exe ./cmd/gwir
